    ...
    <div role="img" aria-label={ariaLabel} className="space-y-1.5">
      {data.map(({ label, value }) => (
        <div key={label} className="flex items-center gap-3">
          <span className="w-12 shrink-0 text-right text-xs font-semibold text-steel-500">
            {label}
          </span>
          <div className="relative h-5 flex-1">
            <div
              className="absolute inset-y-0.5 left-0 rounded-r bg-lake-600"
              style={{ width: `${(value / max) * 100}%`, minWidth: "2px" }}
            />
            <span
              className="absolute inset-y-0 flex items-center pl-2 text-xs font-medium text-steel-600"
